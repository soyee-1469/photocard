...
class PhotocardStudioPage extends StatefulWidget {
  const PhotocardStudioPage({
    super.key,
    this.startSealed = false,
    this.onOpened,
  });

  /// 구매 직후 미개봉 팩부터 보여 줄 때 사용.
  final bool startSealed;

  /// 오픈 애니메이션이 끝나면 호출. 서비스 플로우에서 결과 화면으로 넘길 때 사용.
...
class _PhotocardStudioPageState extends State<PhotocardStudioPage>
  ...
  Widget build(BuildContext context) {
    ...
    final bool serviceFlow = widget.onOpened != null;
    final String? actionLabel = switch (_phase) {
      _Phase.idle => '포토카드 뽑기',
      _Phase.sealed => '오픈하기',
      _Phase.revealed => serviceFlow ? null : '다시 뽑기',
      _Phase.opening => null,
    };
                ...
                if (actionLabel == null)
                  Opacity(
                    ...
                    child: _PrimaryButton(
                      label: _phase == _Phase.revealed ? '공개 중…' : '오픈 중…',
                      onTap: () {},
                    ),
                  )
                else
